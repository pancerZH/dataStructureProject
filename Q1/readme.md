...
![image](./master/Q1/image/build.PNG)
...
- #### 修改操作
  ...
  ```c++
  bool Database::fix(const string num, const string name, 
	                   const string sex, const string age, const string job)
	{
		Student* temp = find(num);

		if (temp != NULL)//找到考生
		{
			temp->changeName(name);//调用Student对象的修改函数
			temp->changeSex(sex);
			temp->changeAge(age);
			temp->changeJob(job);
			return true;
