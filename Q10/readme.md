...
对两个有序数组的合并操作是极容易的。由于我们没有采用链表的形式储存元素，所以我们需要一个辅助数组储存合并之后的结果；而且，为了避免每次进行合并操作都要申请新的空间，我们在算法开始之初，就申请一个足够大的数组空间用于存放中间结果，这就节省了多次new运算的开销。  
代码如下  
```c++
void Sort::merge()
{
...
由于桶排序实际上不存在相同元素的先后问题，所以姑且可以认为这是一种稳定的排序算法。
